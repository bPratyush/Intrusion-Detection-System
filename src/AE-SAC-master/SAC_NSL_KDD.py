...
class RLenv(data_cls):
    def __init__(self, train_test, **kwargs):
        ...
        self.cluster_seed = kwargs.get('cluster_seed', 42)
        self.n_alert = kwargs.get('n_alert', 6)
        self.alert_clusters = {}
    ...
    def reset(self):
        ...
        # Train GMM model for each attack type
        for attack_type in self.attack_types:
            X_pos = self.states[self.labels[attack_type] == 1]
            self.alert_clusters[attack_type] = GaussianMixture(n_components=self.n_alert, random_state=self.cluster_seed).fit(X_pos)
    ...
    def act(self, defender_actions, attack_actions):
        ...
        # Print alert labels for each attack type
        if self.alert_clusters is not None:
            for attack_type in self.attack_types:
                X_pos = self.states[self.labels[attack_type] == 1]
                y_pos = self.alert_clusters[attack_type].predict(X_pos) + 1
                print("Alert labels of {} attacks: {}".format(attack_type, collections.Counter(y_pos)))
    ...
    def get_states(self, attacker_actions):
        ...
        # Add alert information to the states for each attack type
        if self.alert_clusters is not None:
            for attack_type in self.attack_types:
                X_pos = self.states[self.labels[attack_type] == 1]
                y_pos = self.alert_clusters[attack_type].predict(X_pos) + 1
                self.states[attack_type + '_alert_labels'] = np.zeros(len(self.states))
                for i, attack in enumerate(attacker_actions):
                    self.states.loc[self.states.index[i], attack_type + '_alert_labels'] = y_pos[i]
